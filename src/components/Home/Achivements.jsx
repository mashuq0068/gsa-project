const Achievements = () => {
    ...
    return (
      <div className="bg-gray-900 relative min-h-screen flex flex-col items-center justify-center px-5 text-center">
            <div className="absolute top-10 left-0 right-0 h-[5px] bg-orange-400"></div> {/* Top Border */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-5xl">
          {achievements.map((achievement, index) => (
            <div
              key={index}
              className="rounded-2xl shadow-lg p-8 hover:shadow-2xl transition-all duration-300"
            >
              <h3 className="md:text-6xl text-3xl font-bold text-white  mb-4">{achievement.number}</h3>
              <p className=" text-lg text-white font-light">{achievement.description}</p>
            </div>
          ))}
        </div>
        <a
